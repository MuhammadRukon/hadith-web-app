...
const BookTabWithCategory = ({ item, setHadiths }) => {
  ...
  return (
    ...
    >
      <Link to={`/hadith-books/${item._id}`}>
        <h2 className={`${isSelected && "font-semibold"} text-sm lg:text-base pl-1 lg:pl-3 text-nowrap capitalize`}>
          {item.name[lang]}
        </h2>
      </Link>
      {isSelected && item?.chapters?.length > 0 && (
        ...
        >
          {item?.chapters?.map(chapter => {
           return <option  value={chapter?._id}>{chapter?.name[lang]}</option>
          })}
        </select>
      )}
    </div>
